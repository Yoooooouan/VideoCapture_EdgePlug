...
| 功能 | 说明 |
| --- | --- |
| 🔎 URL 搜索栏 | 弹窗内直接输入网址，后台自动打开并检测该页视频，无需先手动访问 |
| 🎬 多站点支持 | B 站（b站/哔哩哔哩）、抖音、以及所有 HLS/DASH/直链站点 |
| 📡 智能抓取 | `webRequest` 拦截 + `<video>` 元素检测 + B 站 `__playinfo__` 提取 + HTML 静态扫描 |
...
1. 点击工具栏 📹 图标打开弹窗，在顶部搜索栏输入目标网址（可省略 `https://`，如 `www.bilibili.com/video/BVxxxx`）
2. 点击「🔍 检测」或按回车，扩展会在**后台标签页**自动打开该页面并分析（约 5~15 秒）
3. 检测完成后展示该页面的视频列表，下载操作与方式一完全一致（同样走后台下载，关闭弹窗不中断）
4. 顶部显示「🔎 来源」条，点击「← 当前页面」可随时切回当前标签页的检测结果

> 检测完成后探测标签页会自动关闭（下载由 offscreen 按 URL 执行，不依赖页面存活）。部分需要播放才加载视频流的网站可能检测为空，此时建议用方式一。
...
### 4. URL 搜索栏扫描（SCAN_URL）
- 输入网址后，`background.js` 以 `active: false` 打开**后台标签页**加载目标页面
- 复用现有采集链路：content script `<video>` 元素检测、`webRequest` 媒体拦截、B 站 `__playinfo__` 提取
- 并行执行**静态 HTML 扫描**兜底：`fetch` 页面源码，正则提取 `<video>/<source>/<audio>` 标签、`og:video` meta、以及 JS/JSON 内嵌的 `.mp4/.m3u8/.m4s` 等裸地址（覆盖懒加载与动态站点）
- 扫描结果（state）搬到虚拟 key `scan_<tabId>` 后关闭探测标签页；下载走 offscreen 按 URL 执行，不依赖页面存活
- popup 的下载/预估请求携带该虚拟 tabId，`findMediaAndState()` 优先按指定 key 定位媒体（找不到时全局兜底），与现有"当前页面"链路互不干扰
...
| URL 扫描（动态站点） | 部分网站检测为空 | 后台标签页 autoplay 被浏览器阻止，不播放则不加载视频流；此类站点建议先打开页面播放再用方式一 |
